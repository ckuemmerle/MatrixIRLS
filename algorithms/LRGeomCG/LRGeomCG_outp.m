function [x,histout,fail,Xout,itc,time] = LRGeomCG(prob, opts, x0)
%
% Implementation of the Riemannian optimization-based algorithm for matrix
% completion, 'Low-rank matrix completion by Geometric CG' ('LRGeomCG') [1].
% Code by Bart Vandereycken, 2011-2012, cf. 
% http://www.unige.ch/math/vandereycken/matrix_completion.html
% =========================================================================
% [1] Bart Vandereycken, Low-Rank Matrix Completion by Riemannian
...
if isfield(opts,'recsys') && opts.recsys == 1
    recsys = 1;
    if isfield(prob,'train') && isfield(prob,'test')
    else
...
for itc=1:opts.maxit
  ...
  if recsys 
       YY=xc_new.V;
       XX=xc_new.U*diag(xc_new.sigma);
       RMSE(itc) = MatCompRMSE(XX,YY,eye(size(XX,2),size(YY,2)),...
           prob.test.row, prob.test.col, prob.test.data);
       RMSE_train(itc) = MatCompRMSE(XX,YY,eye(size(XX,2),size(YY,2)),...
           prob.train.row, prob.train.col, prob.train.data);
       MABS(itc) = MatCompMABS(XX,YY,eye(size(XX,2),size(YY,2)),...
           prob.test.row, prob.test.col, prob.test.data);
       MABS_train(itc) = MatCompMABS(XX,YY,eye(size(XX,2),size(YY,2)),...
           prob.train.row, prob.train.col, prob.train.data);
  end
...
if isfield(opts,'saveiterates') && opts.saveiterates == 1
    ...
    x.Xout = {{xc_new.U*diag(xc_new.sigma),xc_new.V}};
    Xout= x;
end
if recsys
   Xout.RMSE      = RMSE;
   Xout.RMSE_train= RMSE_train;
   Xout.MABS      = MABS;
   Xout.MABS_train= MABS_train;
end
time=time(1:itc);
