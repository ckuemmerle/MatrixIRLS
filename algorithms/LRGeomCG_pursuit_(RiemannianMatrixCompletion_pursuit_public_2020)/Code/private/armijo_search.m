function [xt,ft, gradt, succ,numf,iarm,lambda] = armijo_search(prob, xc,fc,gc,dir,lambda)
...
[ft, xt, store, gradt] = F_line_search(prob,xc,dir, lambda);
%xt = moveEIG(prob,xc,dir,lambda); ft = F(prob,xt);
numf = numf+1; iarm = 0;
...
while(ft > fgoal)    
    ...
    [ft, xt, store, gradt] = F_line_search(prob,xc,dir, lambda, store);
    numf = numf+1; qc=ft;
    if(iarm > MAX_IARM)
        succ = false;        
        return
