function [xc,ithist] = truncated_SD(prob, opts)
...
while cur_rank <= opts.max_rank
    ...
    elseif strcmpi(opts.search_strategy, 'full_space')
        xc.U = [xc.U U];
        xc.V = [xc.V V];        
        [xc.S,A_S,ind_i_S,ind_j_S,Q_S,R_S] = getoptS_full_space(prob,xc.U,xc.V,A_S,ind_i_S,ind_j_S,Q_S,R_S);
...
function [S,A,ind_i,ind_j,Qa,Ra] = getoptS_full_space(prob,L,R,A,ind_i,ind_j,Qa,Ra)
% Can only be used if L and R are the same expect for the last column!!!      
    ...
    if isempty(A)  
        ...
        [Qa,Ra] = qr(A,0);
        Svec = Ra\(Qa'*prob.data);
    else
        Anew = [];
        for i=1:r
            j = r;
            Anew = [Anew partXY(L(:,i)', R(:,j)', prob.Omega_i, prob.Omega_j, prob.m)'];
            ind_i = [ind_i i]; ind_j = [ind_j j];
        end
        for j=1:r-1
            i = r;
        ...
        [Svec,Qa,Ra] = solve_lsq_updated(Qa,Ra,Anew,prob.data);
    end
...
function [x,Qa,Ra] = solve_lsq_updated(Qa,Ra,B,y)
% this is probably not stable (resembles classical GS) better use a QR update 
% (but not in Matlab, see fortran code of Hammarling or Kressner)
Rab = Qa'*B; B2 = B - Qa*Rab;
[Qb,Rb] = qr(B2,0);
Qa = [Qa Qb];
Ra = [Ra Rab; zeros(size(Rb,1),size(Ra,2)) Rb];
x = Ra\(Qa'*y);
end
