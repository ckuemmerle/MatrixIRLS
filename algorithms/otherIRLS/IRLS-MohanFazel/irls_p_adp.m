...
function [Xr,outs] = irls_p(prob,opts)
%% Get PARAMETERS
...
%% Run algorithm
if verbose > 0
    fprintf(['Run IRLS-p with p=%.2f and smoothing parameter rule "%-11s"...\n'], ...
    p,mode_eps);
end
while(k<N0)
    [Xnew,~,terr,l] = grad_proj(X0_revealed,L,Xnew,V,D1,d1,d2,alpt,betat,500);
    if saveiterates
        X{k}=Xnew;
    end
    if strcmp(mode_eps,'auto_decay') || strcmp(mode_eps,'iter_diff')
    ...
    eps(k)   = sqrt(gam);
    if verbose
        fprintf(['k: %3d rankest: %3d gamma: %.3e relchg: %.2e\n'], ...
        k,count,gam,rel_chg);  
    else
        if(mod(k,20) == 0)
            fprintf('.');
